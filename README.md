...
passing the proper flags to clang to build an iOS/Mac OS X application. This
tool has support to configure a default OS target on compilation time, but
also allows to configure the setup on run time. Configuration works by setting
the following variables to appropriate values:
...
* `TARGET`: The target to configure the wrapper for. `build-cctools-port.sh`
            ...
            - `arm-apple-darwin11`
            - `ppc-apple-darwin8`
* `SDK_DEFAULT`: The path to the SDK which will be used by default.
                 The SDK's basename needs to include the OS version the SDK is
                 for. It also needs to include the OS name
                 (iPhoneOS vs MacOSX).
* `PARALLELISM`: Allows to specify the number of jobs `make` shall run
                 simultaneously. This is directly passed to `make` via the
                 `-j` parameter.
                 *Default:* 2.
* `MIN_VER_DEFAULT`: The minimum OS version the build is targeting.
                     *Default:* Version of the default SDK.
...
TARGET=arm-apple-darwin9 SDK_DEFAULT="$IOS_TOOLCHAIN_BASE/sdks/iPhoneOS4.2.sdk/" MIN_VER_DEFAULT=3.1 ./build-clang-wrapper.sh
TARGET=arm-apple-darwin11 SDK_DEFAULT="$IOS_TOOLCHAIN_BASE/sdks/iPhoneOS7.1.sdk/" ./build-clang-wrapper.sh
TARGET=ppc-apple-darwin8 SDK_DEFAULT="$IOS_TOOLCHAIN_BASE/sdks/MacOSX10.2.8.sdk/" ./build-clang-wrapper.sh
```
...
program targeting iOS or Mac OS X are as follows:
...
`clang-wrapper` allows you to override the SDK and minimum Mac OS X version
targeted by setting the following environment variables accordingly:

* `MACOSX_SDK`: Path to the root of the SDK.
* `MACOSX_DEPLOYMENT_TARGET`: Minimum Mac OS X version targeted.
...
**NOTE**: Specifying `-mmacosx-version-min=xx.yy` manually on command line
overrides any `clang-wrapper` defaults and the `MACOSX_DEPLOYMENT_TARGET`
variable.
...
* patches/clang/0001-macho-armv6k-sub-type.patch
...
* patches/clang/0002-legacy-ios-sdk-support.patch

Allows clang++ to pick up C++ headers for ancient 3.1.3 SDKs.

* patches/clang/0003-legacy-osx-sdk-support.patch

Adds support for really old Mac OS X SDKs and target versions. For 10.2
and 10.3 STLport is used as STL library.

* patches/clang/0004-macho-indirect-symbol-table-fix.patch

Fixes a bug in LLVM's MachO object writer which causes incorrect MachO output
for PowerPC object code. The MachO indirect symbol table is not properly
created without this patch, which causes wrong symbols to be picked up on
linking.
